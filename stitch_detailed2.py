...
try:
    FEATURES_FIND_CHOICES['surf'] = cv.SIFT_create
...
def main(args):
    ...
    _main(args, imgs, img_names)

def _main(args, imgs, img_names):
    ...
    if save_graph:
        with open(args.save_graph, 'w') as fh:
            fh.write(cv.detail.matchesGraphAsString("a.jpg", p, conf_thresh))
...
def incremental(args):
    imgs, img_names = load_imgs()
    new_img = _main(args, imgs[:2], img_names[:2])
    for i in range(2, len(imgs)):
        new_img = _main(args, [new_img.astype(np.uint8), imgs[i]], ['result.jpg', img_names[i]])
