...
class Panorama:
    ...
    def get_interest_points(self, show=False):
        ...
        if show:
            ...
            draw_params = dict(matchColor = (0,255,0),
                               ...
                               flags = 0)
            img3 = cv2.drawMatchesKnn(self.img1["array"], keypts1, self.img2["array"], keypts2, matches, None, **draw_params)
            # plt.imshow(img3)
            # plt.show()
            if img3.shape[2] == 4:
            ...
            cv2.imwrite('../data/match.jpg', img3)
    ...
    def putative_matches(self, factor=0.7):
        ...
        if len(p_matches) <= 4:
            ...
            return False
        self.img1.update({'matches' : np.array([list(self.img1["keypts"][i].pt) for (_,i) in p_matches])})
        self.img2.update({'matches' : np.array([list(self.img2["keypts"][i].pt) for (i,_) in p_matches])})
...
if __name__ == '__main__':
    ...
    pan = Panorama(imgs[0], imgs[1])
    pano = pan.construct_panorama(show=True)
    for i in range(2, len(image_names)):
        plt.imshow(pano)
        plt.show()
        pan.img1["array"] = pano
        pan.img2["array"] = imgs[i]
        pano = pan.construct_panorama(show = True)
    ...
    cv2.imwrite("../data/pano1.jpg", pano)